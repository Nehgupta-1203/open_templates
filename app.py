...
from utils.ssh_client_wrapper import ssh_connection, run_command_in_ssh, reconnect_ssh_if_needed
...
def deploy_image():
    ...
    # tag = docker_options.get('tag', 'latest')
    container_name = docker_options.get('container_name', image_name) # customized name for the container
    ports = docker_options.get('ports', [])
    env_vars = docker_options.get('env_vars', [])
    entrypoint = docker_options.get('entrypoint', None)
    num_of_cpus = docker_options.get('num_of_cpus')
    memory_limit = docker_options.get('memory_limit')
    shm_size = docker_options.get('shm_size')
    runtime = docker_options.get('runtime', 'normal')
    gpus = docker_options.get('gpus', None)
    restart_policy = docker_options.get('restart_policy')
...
def verify_vm_is_connected():
    ...
    if not is_connected_to_vm:
        try:
            reconnect_ssh_if_needed(session.get('ssh_client'))
            session['vm_connected'] = True
        except Exception as ex:
